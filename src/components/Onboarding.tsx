...
export function Onboarding({ onComplete, initialBrandName }: OnboardingProps) {
  ...
  const autoGenerated = useMemo<PromptDraft[]>(() => {
    ...
    if (enabledTypes.category) {
      out.push({ type: 'category', text: `best ${categoryLabel} tools` })
      out.push({ type: 'category', text: `top ${categoryLabel} platforms in 2025` })
      out.push({ type: 'category', text: `which ${categoryLabel} should I choose` })
    }
    ...
    return out
  }, [enabledTypes, categoryLabel, brandLabel, competitors])
